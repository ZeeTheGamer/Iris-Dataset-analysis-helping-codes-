...
for a in all_columns:
  for b in all_columns:
      if a!=b:
          ...
          tempArr =  [a_temp,b_temp,round(accuracy, 3)]
          
          resultsArra.append(tempArr)

#NOW WRITING ON CSV FILE

with open('2FeatureResults/Resut_with_n_'+str(neighborsNumber)+'_testSize_'+str(test_sizeNumber)+'_2_features.csv', mode='w',newline='') as csv_file:
    fieldnames = ['Feature A', 'Feature B', 'Accuracy']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    
    writer.writeheader()
    for row in resultsArra:
        #writer.writerow({row[]})
        writer.writerow({'Feature A': row[0], 'Feature B': row[1], 'Accuracy': row[2]})
        #print(row[0])
